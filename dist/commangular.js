...
(function (window, angular, undefined) {
  ...
  var injections = {};
  var injectionSources = {};

  commangular.inject = function inject(scope, name, target) {
    if (!injections[name]) {
      injections[name] = [];
    }
    injections[name].push({
      scope : scope,
      target: target
    });
    scope[target] = injectionSources[name];
  };

  commangular.getInjectionsByName = function getInjectionsByName(name) {
    return injections[name];
  };

  commangular.doInjection = function doInjection(name, value) {
    _.each(commangular.getInjectionsByName(name),
      function (injection) {
        injection.scope[injection.target] = value;
      });
  };

  commangular.registerInjectionSource =
    function registerInjectionSource(name, value) {
      injectionSources[name] = value;
      commangular.doInjection(name, value);
    };
  ...
  commangular.create = function (commandName, commandFunction, commandConfig) {
    if (commandName === void(0)) {
      console.warn(
        'Warning: creating a command with an undefined name. Should be fixed!\ncommand: ' +
        JSON.stringify(commandFunction) +
        '\nAdd a breakpoint to this line and find the problem in the stack trace.'
      );
    }

    if (commandCache[commandName] === true) {
      throw 'Error: commandName: "' + commandName +
            '" was already created. You should not create this command again. Fix this!'
    }

    commandCache[commandName] = true;
  ...
  commangular.aspect = function (aspectDescriptor, aspectFunction, order) {
    ...
    if (!/(\bBefore\b|\bAfterExecution\b|\bAfter\b|\bAfterThrowing\b|\bAround\b)/.test(poincut)) {
      throw new Error('aspect descriptor ' + aspectDescriptor +
                      ' contains errors');
    }
    aspects.push({
      poincut       : poincut,
  ...
  commangular.resolver = function (commandName, resolverFunction) {
    var aspectResolverFunction = [
      'lastResult', 'processor', '$injector',
      function (lastResult, processor, $injector) {
        return {
          execute: function () {
            var result = $injector.invoke(resolverFunction, this,
              {result: lastResult});
  ...
  commangular.build = function () {
    ...
    }(aspects, commandNameString, commands)(eventAspects, eventNameString,
      eventInterceptors));
  };
  ...
  CommandDescriptor.prototype.add = function (command) {
    this.descriptors.push((angular.isString(command)) ?
                          new CommandDescriptor('E', commands[command]) :
                          command);
    return this;
  };
  ...
  CommandDescriptor.prototype.to = function (command) {
    this.descriptors[this.descriptors.length - 1].commandDescriptor =
      (angular.isString(command) ?
       new CommandDescriptor('E', commands[command]) :
       command);
    return this;
  };
  ...
  function CommandContext(data, $q, $injector, $parse) {
    ...
    this.processFlow = function (descriptor) {
      ...
      (function flowChain() {
        ...
        if (typeof result !== 'boolean') {
          throw new Error('Result from expresion :' + descriptor.expresion +
                          ' is not boolean');
        }
        if (result) {
        ...
        .then(function () {
          ...
          try {
            if (descriptor.command.interceptors['Around']) {
              ...
              var command = self.instantiate(descriptor.command.commandFunction,
                true);
      ...
      switch (poincut) {
        case 'Around' :
        {
          ...
          angular.forEach(interceptors[poincut], function (value) {
            processor =
              new AroundProcessor(value.func, processor, self, deferred, $q);
          });
        ...
        default :
        {
          var processor = this.contextData.processor =
            new InterceptorProcessor(self, deferred);
          ...
          (function invocationChain() {
            try {
              ...
              var interceptor = self.instantiate(interceptors[poincut][x++].func,
                false);
              $q.when(self.invoke(interceptor.execute,
    ...
    this.instantiate = function (funct, isCommand) {
      ...
      if (isCommand) {
        this.currentCommandInstance = instance;
      }
      return instance;
    };
    ...
    this.exeOnResult = function (result) {
      ...
      if (this.currentCommandInstance &&
          this.currentCommandInstance.hasOwnProperty('onResult')) {
    ...
    this.exeOnError = function (error) {
      ...
      if (this.currentCommandInstance &&
          this.currentCommandInstance.hasOwnProperty('onError')) {
  ...
  angular.module('commangular', [])
    .provider('$commangular', function () {
      return {
        $get: [
          'commandExecutor', function (commandExecutor) {
            return {
        ...
        mapTo: function (eventName) {
          var interceptorChain = eventInterceptors[eventName] ||
                                 (eventInterceptors[eventName] = {});
          if (!interceptorChain.interceptors) {
            interceptorChain.interceptors = {};
          }
          eventNameString =
            eventNameString.concat("%" + eventName + "%{" + eventName + "}\n");
  ...
  angular.module('commangular')
    .service('commandExecutor', [
      '$q', '$injector', '$parse', '$exceptionHandler',
        ...
        return {
          execute      : function (eventName, data) {
            ...
            return defer.promise.then(function () {
              ...
              var def = $q.defer();
              context.intercept('AfterThrowing',
                interceptors).then(function () {
                  def.reject(error);
                }, function () {
                  defer.reject(error)
                });
    ...
    .config([
      '$provide', function ($provide) {
        $provide.decorator('$rootScope', [
          '$injector', '$delegate', function ($injector, $delegate) {
